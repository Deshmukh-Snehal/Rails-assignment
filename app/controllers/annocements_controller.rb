class AnnocementsController < ApplicationController
  ...
  # POST /annocements or /annocements.json
  def create
    @annocement = Annocement.new(annocement_params)
    # @annocement = current_user.annocements.build(annocement_params)
    respond_to do |format|
      if @annocement.save
        AnnounceMailer.Announcement_created.deliver_later
        format.html { redirect_to annocement_url(@annocement), notice: "Annocement was successfully created." }
  ...
  def update
    respond_to do |format|
      if @annocement.update(annocement_params)
        format.html { redirect_to annocement_url(@annocement), notice: "Annocement was successfully updated." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to annocements_url, notice: "Annocement was successfully destroyed." }
  ...
  private
    ...
    def set_annocement
      @annocement = Annocement.find(params[:id])
    end
